...
import {type Control, Controller, FormProvider, useFormContext, useWatch,} from "react-hook-form";
...
import {
	useCreateUpdateProductSku
} from "@/features/admin/product/hooks/use-create-update-product-sku.tsx";
import type {
...
const PickImages = ({
	...
	return (
		<>
			<div className="flex flex-col gap-y-4">
				{(productImages[0] || filesWithPreview[0]) && (
					<img
						className="w-full h-[400px]! object-cover max-w-full"
						src={productImages[0]?.imageUrl || filesWithPreview[0].url}
						alt={productImages[0]?.imageName || filesWithPreview[0].file.name}
					/>
				)}
					...
					{productImages.map((f) => (
						...
						>
							...
							<Button
								...
								// 	)
								// }
								className="w-4 h-4 rounded-full flex items-center justify-center bg-red-500 text-white hover:bg-red-600 transition-colors !p-4 ml-auto"
								title="Remove"
							>
								<X className="w-4 h-4" />
							</Button>
...
const Buttons = ({
	...
	const disabled =
		action === "update"
			...
			Number(salePrice) === Number(data.salePrice) &&
			Number(quantity) === data.quantity &&
			sku === data.sku &&
			Number(size) === data.size &&
			color === data.color &&
