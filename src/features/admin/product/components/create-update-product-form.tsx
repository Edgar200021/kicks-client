...
type ActionType =
	| { action: "create" }
	| { action: "update"; initialData: AdminProduct };
...
export const CreateUpdateProductForm = ({
	className,
	renderTrigger,
	type,
	onSuccess,
	...
	const { apiValidationErrors } = useHandleError<(keyof CreateProductInput)[]>(
		error || updateError,
	);
	...
	const isUnavailable =
		!serverFilters ||
		!serverFilters.availableBrands.length ||
		!serverFilters.availableCategories.length;
...
const SubmitButton = ({
	...
	const noChanges =
		...
		title === type.initialData.title &&
		description === type.initialData.description &&
		areArraysEqual(tags ?? [], type.initialData.tags) &&
		categoryId === type.initialData.category?.id &&
		brandId === type.initialData.brand?.id &&
		gender === type.initialData.gender;

	return (
		<Button disabled={isLoading || noChanges}>
			{!isLoading
				? `${type.action === "create" ? "Create" : "Update"} Product`
				: "Loading..."}
		</Button>
	);
};
